...
export const ScrollSmootherHeader: React.FC<ScrollSmootherHeaderProps> = ({ className, backgroundImage }) => {
  ...
  // CORREÇÃO DE HIDRATAÇÃO: Estado para garantir que o código só rode no cliente.
  ...
  useEffect(() => {
    // A animação só roda após o componente ser montado no cliente.
    if (!isMounted) return;
    ...
    // Contexto GSAP para limpeza segura das animações
    const ctx = gsap.context(() => {
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub: 1,
          // Adiciona/remove a classe de vidro ao entrar/sair da área de scroll
  ...
  return (
    ...
    >
      ...
      <h1
        ref={titleRef}
        className="text-center uppercase text-[#00e1ff] font-bold font-['Luckiest_Guy'] absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[60] text-shadow-[0_0_2px_rgba(0,0,0,0.3)]"
      ...
      >
        ...
        <div className="hidden md:flex flex-1 items-center gap-1">
          {/* Links de navegação para desktop podem ir aqui */}
        </div>
