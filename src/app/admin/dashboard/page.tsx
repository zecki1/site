...
// Função auxiliar segura para definir valor em um caminho de objeto aninhado
function setNestedValue(obj: unknown, path: string, value: string | string[]): void {
    ...
    for (let i = 0; i < keys.length - 1; i++) {
        ...
        if (!isIndexable(current)) {
            console.error("Caminho inválido para setNestedValue:", path);
            return;
        }
        current = current[key];
    }
    ...
    if (isIndexable(current)) {
        ...
        console.error("Caminho final inválido para setNestedValue:", path);
    }
}
    ...
    return (
        <div>
            ...
            <div className="mt-2 flex items-center gap-4">
                {imageUrl ? (<Image src={imageUrl} alt="Preview" width={128} height={128} className="w-32 h-32 rounded-md object-cover border" />) : (<div className="w-32 h-32 rounded-md border border-dashed flex items-center justify-center bg-muted"><ImageIcon className="h-8 w-8 text-muted-foreground" /></div>)}
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            if (currentUser) {
                ...
                if (docSnap.exists()) { setSiteData(docSnap.data().siteData as SiteData); }
                else { setError("Seus dados de site não foram encontrados. Por favor, contate o suporte."); }
    ...
    const handleInputChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, path: string) => {
        ...
        setSiteData(prevData => {
            ...
            const processedValue = keys[keys.length - 2] === 'typedItems' ? value.split(',').map(item => item.trim()) : value;
    ...
    const onImageRemove = async (path: string, currentUrl: string) => {
        ...
        try {
            ...
            if (storageError.code !== 'storage/object-not-found') { toast.error("Falha ao remover a imagem do armazenamento."); }
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (clientSiteUrl && revalidateToken) {
                ...
                if (!res.ok) { const errorData = await res.json(); throw new Error(errorData.message || `Falha na revalidação: status ${res.status}`); }
            ...
            toast.error("Erro ao publicar alterações.", { description: error.message || "Verifique o console." });
        } finally {
            setIsSaving(false);
        }
    };
            ...
            <div className="container mx-auto p-4 md:p-8">
                ...
                <Accordion type="single" collapsible defaultValue="hero" className="w-full space-y-4">
                    <AccordionItem value="hero" className="border-none rounded-lg overflow-hidden shadow-sm">
                        <AccordionTrigger className="text-xl font-semibold bg-card px-6 py-4 hover:no-underline">Seção Hero (Início)</AccordionTrigger>
                                ...
                                <Tabs defaultValue="ptBR">
                                    ...
                                    {languages.map(lang => (
                                        <TabsContent key={lang} value={lang} className="mt-4">
                                            <div><Label>Textos Animados (separados por vírgula)</Label><Input value={siteData.hero.typedItems[lang].join(', ')} onChange={(e) => handleInputChange(e, `hero.typedItems.${lang}`)} /></div>
                                        </TabsContent>
                                    ))}
                                </Tabs>
                            </div>
                    ...
                    <AccordionItem value="sobre" className="border-none rounded-lg overflow-hidden shadow-sm">
                        <AccordionTrigger className="px-6 py-4 bg-card hover:no-underline">Seção Sobre</AccordionTrigger>
                        <AccordionContent className="bg-card px-6 pb-6 pt-2">
                            <div className="space-y-6">
                    ...
                    <AccordionItem value="portfolio" className="border-none rounded-lg overflow-hidden shadow-sm">
                        <AccordionTrigger className="px-6 py-4 bg-card hover:no-underline">Seção Portfólio</AccordionTrigger>
                        <AccordionContent className="bg-card px-6 pb-6 pt-2">
                                ...
                                {siteData.portfolio.items.map((item, index) => (
                                    <Card key={item.id} className="p-6 relative bg-muted/30">
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader><AlertDialogTitle>Excluir Item?</AlertDialogTitle>
                                                    <AlertDialogDescription>{`Deseja realmente remover o item "${item.titulo.ptBR}"? A imagem associada será excluída permanentemente.`}</AlertDialogDescription>
                                                </AlertDialogHeader>
                    ...
                    <AccordionItem value="contato" className="border-none rounded-lg overflow-hidden shadow-sm">
                        <AccordionTrigger className="px-6 py-4 bg-card hover:no-underline">Seção Contato e Redes Sociais</AccordionTrigger>
                        <AccordionContent className="bg-card px-6 pb-6 pt-2">
                            <div className="space-y-6">
