...
// ... (todo o código da classe Particle e da função hexToRgb permanece o mesmo)
...
// O componente em si não tem 'export' na sua declaração
const CanvasParticles = () => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const highlightColor = "#00e1ff";
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            for (let a = 0; a < particles.length; a++) {
                ...
                if (distanceMouse < 150) {
                    ...
                    ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(particles[a].x, particles[a].y); ctx.lineTo(mouse.x, mouse.y); ctx.stroke();
                }
        ...
        const animate = () => {
            ...
            particles.forEach(p => { p.update(canvas.width, canvas.height); p.draw(ctx, 'rgba(255, 255, 255, 0.8)'); });
            connect();
...
// A CORREÇÃO CRÍTICA: Exportamos como 'default' no final do arquivo.
